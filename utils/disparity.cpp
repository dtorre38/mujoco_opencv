...
void disparity() {
    ...
    //     std::cerr << "Error: Unable to load one or more images." << std::endl;
    //     return 1;
    // }
    ...
    for (int i = 0; i < height_chunks.size() - 1; ++i) {
        std::thread t(compute_disparity, height_chunks[i], height_chunks[i + 1], 0, left.cols - 1, &left, &right, &stereo);
        t.join();
    }
}
