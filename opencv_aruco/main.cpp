...
// #include "../utils/generateCameraMatrix.cpp"
...
int main(int argc, char** argv) {
    ...
    // camera stereo vision - applies to all cameras
    ...
    // Define the desired interval in milliseconds
    ...
    while( !glfwWindowShouldClose(window))
    {
        ...
        int remaining_ms = interval_ms - elapsed_time.count();
        if (remaining_ms > 0) {
            // If the function execution took less than the interval, sleep for the remaining time
            std::this_thread::sleep_for(std::chrono::milliseconds(remaining_ms));
        } else {
            // If the function execution took longer than the interval, log a warning
            std::cout << "Warning: Function execution time exceeds interval" << std::endl;
        }
        ...
        count++;
        cum_error += dist_error;
        mae = cum_error / count;
        printf("MAE = %f \n", mae);

        cum_time += cpu_time_used;
